...
internal static partial class UploadOrchestrationLoggingExtensions
{
    ...
    [LoggerMessage(
        EventId = 5,
        Level = LogLevel.Information,
        Message = "Same Symbol Repo; old format symbols only - uses symbol api to upload at same time"
    )]
    public static partial void SameSymbolRepoOldFormatSymbolsUsingSymbolApiAtSameTime(
        this ILogger<UploadOrchestration> logger
    );
    ...
    [LoggerMessage(
        EventId = 7,
        Level = LogLevel.Information,
        Message = "Same Symbol Repo; mixture - old format using symbol api, new format to primary"
    )]
    public static partial void SameSymbolRepoMexedFormatSymbolsOldSymolApiNewToPrimary(
        this ILogger<UploadOrchestration> logger
    );
}
